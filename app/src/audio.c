...
bool audio_player_init(struct audio_player *player, const char *serial) {
...
bool audio_player_open(struct audio_player *player) {
...
static void audio_player_set_paused(struct audio_player *player, bool paused) {
...
void audio_player_play(struct audio_player *player) {
    audio_player_set_paused(player, false);
}

void audio_player_pause(struct audio_player *player) {
    audio_player_set_paused(player, true);
}
...
// FIXME: Delete aoa
bool audio_forwarding_start(struct audio_player *player, const char *serial) {
    if (!aoa_init()) {
    ...
    if (!aoa_forward_audio(player->serial, true)) {
...
error_disable_audio_forwarding:
    if (!aoa_forward_audio(serial, false)) {
...
void audio_forwarding_stop(struct audio_player *player) {
    ...
    if (aoa_forward_audio(player->serial, false)) {
